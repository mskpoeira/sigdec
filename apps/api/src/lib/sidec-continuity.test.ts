...
test("continuidade fica conforme com replicação e drill dentro dos objetivos",()=>{
  ...
  const result=evaluateSidecContinuity(policy,{
    ...
    latestSuccessfulDrillAt:new Date("2026-09-23T12:00:00.000Z"),
    latestSuccessfulDrillDurationMinutes:5
  },now);
...
test("réplica ausente viola RPO administrativo",()=>{
  const result=evaluateSidecContinuity(policy,{
    ...
    latestSuccessfulDrillAt:new Date(),
    latestSuccessfulDrillDurationMinutes:2
  });
...
test("drill vencido e duração acima do objetivo violam prontidão",()=>{
  ...
  const result=evaluateSidecContinuity(policy,{
    ...
    latestSuccessfulDrillAt:new Date("2026-09-10T12:00:00.000Z"),
    latestSuccessfulDrillDurationMinutes:45
  },now);
  assert.equal(result.overall,"BREACH");
...
test("sem artefatos e sem drill retorna dados insuficientes",()=>{
  const result=evaluateSidecContinuity(policy,{
    ...
    latestSuccessfulDrillAt:null,latestSuccessfulDrillDurationMinutes:null
  });
