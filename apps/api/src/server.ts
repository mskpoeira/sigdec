...
app.get("/health",async()=>({status:"ok",service:"sigdec-api",version:"1.40.0",timestamp:new Date().toISOString()}));
...
app.get("/api/v1",async()=>({name:"SIGDEC API",version:"v1",release:"1.40.0",modules:["auth","ocorrencias","despacho","campo","riscos","monitoramento","alertas","vistorias","documentos","desastres","sco","assistencia-humanitaria","voluntariado","logistica","comunicacoes","s2id","treinamentos","recuperacao","biblioteca","bi","administracao","integracoes","sidec-interoperabilidade","sidec-continuidade","inteligencia-assistiva","auditoria"]}));
