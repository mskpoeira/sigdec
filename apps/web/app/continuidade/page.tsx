...
export default function ContinuidadePage(){
 ...
 const verifyChangeProposal=(proposal:ChangeProposal)=>act(async()=>{
  ...
  if(!window.confirm(`Usar o exercício de ${formatDateTimeBR(exercise.startedAt)} para verificar a mudança da revisão v${proposal.targetPlanVersion}?`))return;
 ...
 return <main className="shell moduleShell">
  <header className="listHeader">
   <div><span className="eyebrow">SIGDEC · CONTINUIDADE SIDEC · {SIGDEC_VERSION_LABEL}</span><h1>Plano de Continuidade e Runbook</h1><p>Versões controladas, responsáveis nominais e exercícios de mesa auditáveis. Esta tela não executa failover real automaticamente.</p></div>
  ...
  <section style={{marginTop:18}}>
   ...
   <div className="dataGrid">{runbookRecommendations.length===0?<div className="infoCard">Nenhuma recorrência atingiu o limiar para recomendar revisão do runbook.</div>:runbookRecommendations.map(item=><article className={item.severity==="CRITICAL"||item.severity==="HIGH"?"warningCard":"card"} key={item.id}>
    ...
    <p>{item.rationale}</p>
    <p>Primeira ocorrência: {formatDateTimeBR(item.firstSeenAt)} · última: {formatDateTimeBR(item.lastSeenAt)}</p>
  ...
  <section style={{marginTop:18}}>
   ...
   <div className="card" style={{marginBottom:12}}>
    ...
    {approvalDelegations.filter(item=>item.status==="ACTIVE"||item.status==="SCHEDULED").length===0?<p><small>Nenhuma delegação vigente ou programada.</small></p>:approvalDelegations.filter(item=>item.status==="ACTIVE"||item.status==="SCHEDULED").map(item=><p key={item.id}><small><strong>{item.status}</strong> · {item.delegatorName??item.delegatorMatricula} → {item.delegateName??item.delegateMatricula} · {formatDateTimeBR(item.validFrom)} até {formatDateTimeBR(item.validUntil)} · {item.reason} <button type="button" className="secondaryLink" disabled={busy} onClick={()=>void revokeApprovalDelegation(item)}>Revogar</button></small></p>)}
   </div>
   ...
   {changeMetrics&&changeMetrics.byRecurrence.length>0&&<div style={{marginTop:12}}>
    ...
    <div className="dataGrid">{changeMetrics.byRecurrence.map(item=><article className={item.regressed>0?"warningCard":"card"} key={item.recurrenceKey}>
     ...
     <p><small>Última proposta: {formatDateTimeBR(item.lastProposalAt)}</small></p>
    </article>)}</div>
   </div>}
  </section>
  ...
  <section style={{marginTop:18}}>
   ...
   <div className="dataGrid">{changeProposals.length===0?<div className="infoCard">Nenhuma proposta de mudança registrada.</div>:changeProposals.map(proposal=><article className={proposal.status==="PROPOSED"?"warningCard":"card"} key={proposal.id}>
    ...
    {proposal.approvals?.map(item=>{const expired=item.decision==="APPROVED"&&(!item.validUntil||new Date(item.validUntil).getTime()<=Date.now());return <p key={item.id}><small>{item.decision}{expired?" · VENCIDA":""} · {item.decidedByName??"Usuário"}{item.delegated?" · delegado por "+(item.authorityUserName??"autoridade"):""} · decisão {formatDateTimeBR(item.decidedAt)}{item.validUntil?" · válida até "+formatDateTimeBR(item.validUntil):""}{item.revalidatedAt?" · revalidada "+formatDateTimeBR(item.revalidatedAt):""} · {item.notes}</small></p>})}
    ...
    {proposal.appliedAt&&<p>Aplicada em {formatDateTimeBR(proposal.appliedAt)}{proposal.appliedByName?" · "+proposal.appliedByName:""}</p>}
    {proposal.verifiedAt&&<p>Verificada em {formatDateTimeBR(proposal.verifiedAt)}{proposal.verifiedByName?" · "+proposal.verifiedByName:""}{proposal.verificationExerciseResult?" · exercício "+proposal.verificationExerciseResult:""}</p>}
    {proposal.reportSealed&&<p><strong>Relatório selado:</strong> {proposal.reportSealedAt?formatDateTimeBR(proposal.reportSealedAt):"sim"}{proposal.reportSealedByName?" · "+proposal.reportSealedByName:""} · SHA-256 {proposal.reportHash?.slice(0,16)}… · chave {proposal.reportSealKeyId}</p>}
    {proposal.reportArchived&&<p><strong>WORM:</strong> arquivado em {proposal.reportArchivedAt?formatDateTimeBR(proposal.reportArchivedAt):"—"} · {proposal.reportArchiveHealthy?"íntegro":"aguardando/verificação com problema"}{proposal.reportArchiveRetainUntil?" · retenção até "+formatDateTimeBR(proposal.reportArchiveRetainUntil):""}{proposal.reportArchiveLegalHold?" · legal hold ativo":""}{proposal.reportArchiveVerifiedAt?" · verificado "+formatDateTimeBR(proposal.reportArchiveVerifiedAt):""}</p>}
    {proposal.reportArchiveErrorMessage&&<p><strong>Arquivo WORM:</strong> {proposal.reportArchiveErrorMessage}</p>}
    {proposal.reportReplicaCreated&&<p><strong>Réplica WORM:</strong> {proposal.reportReplicaHealthy?"íntegra":"aguardando/verificação com problema"}{proposal.reportReplicatedAt?" · criada "+formatDateTimeBR(proposal.reportReplicatedAt):""}{proposal.reportReplicaRetainUntil?" · retenção até "+formatDateTimeBR(proposal.reportReplicaRetainUntil):""}{proposal.reportReplicaLegalHold?" · legal hold ativo":""}{proposal.reportReplicaVerifiedAt?" · verificada "+formatDateTimeBR(proposal.reportReplicaVerifiedAt):""}</p>}
  ...
  <section style={{marginTop:18}}>
   ...
   {active&&<article className="card" style={{marginTop:14}}>
    ...
    <p><strong>Ativada:</strong> {active.activatedAt?formatDateTimeBR(active.activatedAt):"—"}{active.activatedByName?" · "+active.activatedByName:""}</p>
  ...
  <section style={{marginTop:18}}>
   ...
   <div className="dataGrid" style={{marginTop:14}}>{exercises.filter(x=>x.status!=="IN_PROGRESS").slice(0,12).map(ex=><article className={ex.result==="FAIL"?"warningCard":"card"} key={ex.id}><h2>{"Runbook v"+ex.planVersion+" · "+ex.status}</h2><p><strong>{ex.result?resultLabels[ex.result]??ex.result:"Sem resultado"}</strong> · {formatDateTimeBR(ex.startedAt)}</p><p>{ex.scenario}</p><p>Concluídas {ex.summary.completed}/{ex.summary.total} · puladas {ex.summary.skipped} · falhas {ex.summary.failed}</p>{ex.notes&&<p>{ex.notes}</p>}<div className="headerActions"><a className="secondaryLink" href={`${API}/api/v1/sidec/continuity/exercises/${ex.id}/report.pdf`}>Baixar relatório PDF</a>{ex.status==="COMPLETED"&&<button type="button" className="primaryButton" disabled={busy} onClick={()=>openAar(ex)}>Abrir AAR</button>}</div></article>)}</div>
  </section>
  ...
  <section style={{marginTop:18}}><h2>Histórico de versões</h2><div className="dataGrid">{(runbook?.revisions??[]).map(item=><article className="card" key={item.id}><h2>{"v"+item.version+" · "+item.status}</h2><p>{item.title}</p><p>Atualizada em {formatDateTimeBR(item.updatedAt)}</p></article>)}</div></section>
 </main>
}
