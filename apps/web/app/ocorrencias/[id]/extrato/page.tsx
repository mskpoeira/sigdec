...
export default function ExtratoOcorrenciaPage(){
 ...
 return <main className="shell moduleShell">
  ...
  <section className="detailGrid">
   ...
   <article className="card detailCard"><h2>Local e datas</h2><p>{[i.addressLine,i.neighborhood].filter(Boolean).join(" · ")||"Endereço não informado"}</p>{i.referencePoint&&<p>Referência: {i.referencePoint}</p>}<p>Aberta em {new Date(i.createdAt).toLocaleString("pt-BR")}</p><p>Extrato gerado em {new Date(data.generatedAt).toLocaleString("pt-BR")}</p></article>
  </section>

  <section className="detailSection"><div><span className="eyebrow">DESPACHOS</span><h2>Recursos empenhados</h2></div>{data.dispatches.length===0?<div className="infoCard">Nenhum despacho.</div>:<div className="dataGrid">{data.dispatches.map((x:any,idx)=><article className="card" key={idx}><h2>{x.teamCode} · {x.teamName}</h2><p>{x.vehicleCode||"Sem viatura"} · {x.status}</p><p>{new Date(x.dispatchedAt).toLocaleString("pt-BR")}</p>{x.notes&&<p>{x.notes}</p>}</article>)}</div>}</section>
