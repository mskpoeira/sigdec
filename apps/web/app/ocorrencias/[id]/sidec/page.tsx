...
export default function SidecExportsPage(){
 ...
 async function runRestoreDrill(item:SidecExport,destination:"PRIMARY"|"REPLICA"){
  setBusy(true);setMessage("");
  try{
   const result=await request(`/api/v1/sidec-exports/${item.id}/resilience/drill`,{
    method:"POST",body:JSON.stringify({destination})
   });
   setMessage(result?.success
    ?`Drill de restauração ${destination==="PRIMARY"?"do primário":"da réplica"} concluído: SHA-256, tamanho e ZIP confirmados.`
    :`Drill de restauração ${destination==="PRIMARY"?"do primário":"da réplica"} detectou falha.`);
   await load();
  }catch(error){setMessage(error instanceof Error?error.message:"Falha ao executar drill de restauração.");}
  finally{setBusy(false)}
 }
 ...
 return <main className="shell moduleShell">
  ...
  <section className="detailSection">
   ...
   <div className="dataGrid">{readiness?.availableDocuments.map(doc=><label className="card" key={doc.id} style={{cursor:"pointer"}}>
    ...
    <h2>{doc.number??"Sem número"} · {doc.title}</h2><p>{doc.documentType} · revisão {doc.revision} · emitido em {formatDateTimeBR(doc.issuedAt)}</p>{readiness?.requiredDocumentIds?.includes(doc.id)&&<small>Selecionado automaticamente para cumprir requisito documental.</small>}
  ...
  <section className="detailSection">
   ...
   {items.length===0?<div className="infoCard">Nenhum pacote gerado para esta ocorrência.</div>:items.map(item=>
    <article className="card" key={item.id}>
     ...
     <p>Gerado em {formatDateTimeBR(item.createdAt)}</p>
