 ...
 return <main className="shell moduleShell">
  ...
  {data&&<>
   ...
   <section style={{marginTop:24}}>
    ...
    {data.plan.status!=="DRAFT"&&<p><small>Checklist bloqueado nesta versão aprovada/ativa; alterações estruturais exigem nova revisão.</small></p>}<form className="incidentForm compactForm" onSubmit={addChecklist}><h3>Adicionar item de checklist</h3><div className="formGrid"><label>Nível<select name="level" defaultValue={data.plan.currentLevel}>{["NORMAL","OBSERVATION","ATTENTION","ALERT","EMERGENCY"].map(x=><option key={x} value={x}>{levelName[x]}</option>)}</select></label><label>Procedimento<input name="title" required/></label><label><input type="checkbox" name="required" defaultChecked/> Obrigatório</label></div><button className="secondaryLink" disabled={busy||data.plan.status!=="DRAFT"}>Adicionar</button></form>
   </section>
